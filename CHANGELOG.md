...
### 新增
- **包裹图片**：博主详情新增包裹图功能，可上传多张（每人上限 20 张），缩略图网格展示，点击放大查看（灯箱，支持左右切换 / Esc 关闭），支持逐张删除
- 包裹图访客可见（脱敏开启时也照常展示，与主图一致）；上传/删除入口仅管理员可见
- 前端页脚显示当前版本号（读取 VERSION 文件，后端新增 `/api/version`）

### 说明
- 包裹图独立表 `kol_package_photo`，以 uid 关联，独立于金山同步；uid 变化时随主图一并迁移；文件存 `uploads/` 并纳入现有备份
- 本次为 SQLite→SQLite 普通更新，服务器用 `scripts/update.sh` 部署即可
