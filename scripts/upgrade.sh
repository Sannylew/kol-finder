#!/usr/bin/env bash
# KOL Finder 升级脚本（解压新包覆盖方式，适用于非 git 部署）
#
...
#
# 若项目是 git 克隆部署，请优先用 update.sh（git pull 更新，更简单）。
set -euo pipefail
...
echo "  如涉及数据去重，请在后台点【立即同步】后执行："
echo "    cd $TARGET_DIR/backend && sudo ./venv/bin/python dedup.py --apply"
echo "============================================================"
